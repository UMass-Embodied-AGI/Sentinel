...
extern "C" void flood_fill(int h, int w, uint8_t *map, int *filled_map) {
    int *qx, *qy, qh = 0, qt = 0;
    qx = new int[h * w];
    qy = new int[h * w];

    memset(filled_map, 0, h * w * sizeof(int));
    qx[qt] = qy[qt] = 0;
    qt++;
    while (qh < qt) {
        int x = qx[qh], y = qy[qh];
        ...
        for (int i = 0; i < 4; i++) {
            int nx = x + dx[i], ny = y + dy[i];
            if (nx >= 0 && nx < h && ny >= 0 && ny < w && filled_map[nx * w + ny] == 0 && map[nx * w + ny] == 0) {
                filled_map[nx * w + ny] = 1;
                qx[qt] = nx;
                qy[qt] = ny;
                qt++;
            }
